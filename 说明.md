...
You can change colors, fonts, sizes in the `assets/css/layouts/_variables.scss` file.
For each specific theme (light or dark) you can change the variables in `assets/css/dark.scss` and `assets/css/light.scss`.
...
Chalk uses Google Fonts by default. You can change the font in `assets/js/webfontloader.js` and in `404.html`.
